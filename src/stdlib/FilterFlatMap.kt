...
fun main(args: Array<String>) {
    albums.filter { it.year > 1975 }.forEach { println(it.title) }

    albums.flatMap {
        val albumTitle = it.title
        it.tracks.filter { it.durationInSeconds > 270 }
                .map { Pair(albumTitle,it.title) }
    }.forEach { println(it) }

    val firstFiveNumbers = listOf<Int>(1,2,3,4,5)

    val result = firstFiveNumbers.fold(1, fun(i : Int,acc :Int) : Int{
        println(" i is $i and acc is $acc")
        return i * acc
    }  )
